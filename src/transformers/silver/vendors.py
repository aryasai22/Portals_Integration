"""
Silver transformer for CEIPAL vendors.
"""
...
class VendorsTransformer(BaseSilverTransformer):
    """Transform CEIPAL vendors to Silver layer."""

    def __init__(self, spark):
        super().__init__(spark, "vendors")
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        # Clean string fields
        for col_name in ["vendor_code", "vendor_name", "contact_person", "city", "state", "country"]:
        ...
        if "vendor_type" in df.columns:
            df = df.withColumn("vendor_type", upper(col("vendor_type")))
        ...
        df = df.withColumn(
            "IS_ACTIVE",
            col("status").isin(["ACTIVE", "APPROVED"])
        )
